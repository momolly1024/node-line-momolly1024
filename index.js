...
bot.on('message', async (event) => {
  ...
  try {
    ...
    if (event.message.text === 'now_playing') {
      ...
      for (let i = 0; i < data.results.length; i++) {
        msg = '推薦你：'+data.results[rand].title 
      }
    } else if (event.message.text === '熱映中') {
      for (let i = 0; i < data.results.length; i++) {
        msg += data.results[i].title + '\n'
      }
    } else if (event.message.text === '簡介') {
      for (let i = 0; i < 5; i++) {
        msg += `${data.results[i].original_title} ${data.results[i].title}\n電影簡介\n${data.results[i].overview}\n\n`
      }
      ...
      msg = '哈囉尼豪~~ \n請輸入【now_playing】或【熱映中】，查看近期熱播中的電影！\n好啦還是你想知道【機器人在哪】、【機器人長相】、【簡介】(僅顯示前五部電影簡介)、【推薦】隨機推薦你一部電影!'
    }
  } catch (error) {
    msg = '發生錯誤'
  }
